...
struct Yhsdfhgv: View {
 
     
    @ObservedObject var sdfgfgiujhsgf: Hsdhjgvjhcvsd = Hsdhjgvjhcvsd()
     ...
     var body: some View {
        ZStack{
            ...
            if nfsduagiygsdf == "wrrewgegfvdfs" || nfsduagiygsdf == "bbsdbdbdf" {
                if self.powegsduf == "Active Path" || powegsduf == "tttyytykgsydjhf" {
                   ...
                   .onAppear{
                    ...
                    Trsdhjvcb(urehgdf: sdfgfgiujhsgf)
                }
            }
        ...
        if mmbnfsduhfg {
            ...
            if let url = URL(string: "https://pawnqueen.store/activepath/activepath.json") {
                URLSession.shared.dataTask(with: url) { wueyqgfsdh, response, error in
                    ...
                    if let aesdvsd = wueyqgfsdh {
                        if let avevdsv = try? JSONSerialization.jsonObject(with: aesdvsd, options: []) as? [String: Any] {
                            ...
                            if let tywequfghjsdbf = avevdsv["uekjhrthgfgdh"] as? String {
